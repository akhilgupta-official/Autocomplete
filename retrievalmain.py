...
def given_word_count(word):
	...
	query_word_count = """MATCH(a:`%s`) where a.auto_name = '%s' return a.self_count"""%(str(word[0])+str("_auto"),str(word))
...
def rel_given_to_prev(given_word,prev_word):
	...
	probable_words={}
	query_second_word = """MATCH (a:`%s`)-[r:belongs_auto]->(b:`%s`) where b.auto_name =~ '%s.*' AND a.auto_name = '%s' return b"""%(str(prev_word[0])+str("_auto"),str(given_word[0])+str("_auto"),str(given_word[0]),str(prev_word))
	#print "try"
	...
	for i in range(len(ret)):
		...
		rel_score="""MATCH(a:`%s`)-[r:belongs_auto]->(b:`%s`) where a.auto_name = '%s' AND b.auto_name = '%s' return r.auto_score, b.self_count"""%(str(prev_word[0])+str("_auto"),str(name[0])+str("_auto"),str(prev_word),str(name))
...
def first_word(first_word):
	...
	if len(first_word)<3:
		...
		for p_word in probable_first_words:
			first_word_query = """MATCH(a:`%s`) where a.auto_name =~ '%s.*' return a.auto_name, a.self_count"""%(str(p_word[0])+str("_auto"),str(p_word))
			res_first_word = graph.cypher.execute(first_word_query)			
...
def second_word(first_word,second_word):
	...
	if len(second_word)<3:
		...
		for trav in probable_second_words:
			...
			query_directed_path_rel = """MATCH(f:`%s`)-[rel:belongs_auto]->(s:`%s`) where f.auto_name= '%s' AND s.auto_name =~ '%s.*' RETURN s ,rel.auto_score """%(str(first_first_aplha)+str("_auto"),str(trav[0])+str("_auto"),str(first_word),str(trav))
...
def third_word(first_word,second_word,third_word):
	...
	if len(third_word)<3:
		query_directed_path_rel = """MATCH(f:`%s`)-[rel:belongs_auto]->(s:`%s`)-[rela:belongs_auto]->(t:`%s`) where f.auto_name= '%s' AND s.auto_name = '%s' AND t.auto_name=~ '%s.*' RETURN t"""%(str(first_first_aplha)+str("_auto"),str(second_first_aplha)+str("_auto"),str(third_first_aplha)+str("_auto"),str(first_word),str(second_word),str(third_word))
		res_directed_path_rel =  graph.cypher.execute(query_directed_path_rel)
		...
		probable_third_words.append(third_word)
	 	...
	 	for trav in probable_third_words:
			#print trav
			query_directed_path_rel = """MATCH(f:`%s`)-[rel:belongs_auto]->(s:`%s`)-[rel1:belongs_auto]->(t:`%s`) where f.auto_name= '%s' AND s.auto_name ='%s' AND t.auto_name=~ '%s.*' RETURN t"""%(str(first_first_aplha)+str("_auto"),str(second_first_aplha)+str("_auto"),str(trav[0])+str("_auto"),str(first_word),str(second_word),str(trav))
...
def fourth_word(first_word,second_word,third_word,fourth_word):
	...
	if len(fourth_word)<3:
		query_directed_path_rel = """MATCH(f:`%s`)-[rel:belongs_auto]->(s:`%s`)-[rel1:belongs_auto]->(t:`%s`)-[rel2:belongs_auto]->(ft:`%s`) where f.auto_name= '%s' AND s.auto_name ='%s' AND t.auto_name= '%s' AND ft.auto_name =~ '%s.*' RETURN ft"""%(str(first_first_aplha)+str("_auto"),str(second_first_aplha)+str("_auto"),str(third_first_aplha)+str("_auto"),str(fourth_word[0])+str("_auto"),str(first_word),str(second_word),str(third_word),str(fourth_word))
	...
	#query_directed_path_rel = """MATCH(p:`autotest`)-[t:belongs]->(n:`autotest`)-[r:belongs]->(m:`autotest`) where p.name= '%s' AND n.name = '%s' AND m.name='%s' RETURN p,n,m"""%(str(first_word),str(second_word),str(trav))
		...
		for trav in probable_fourth_words:
			...
			query_directed_path_rel = """MATCH(f:`%s`)-[rel:belongs_auto]->(s:`%s`)-[rel1:belongs_auto]->(t:`%s`)-[rel2:belongs_auto]->(ft:`%s`) where f.auto_name= '%s' AND s.auto_name ='%s' AND t.auto_name= '%s' AND ft.auto_name =~ '%s.*' RETURN ft"""%(str(first_first_aplha)+str("_auto"),str(second_first_aplha)+str("_auto"),str(third_first_aplha)+str("_auto"),str(trav[0])+str("_auto"),str(first_word),str(second_word),str(third_word),str(trav))
